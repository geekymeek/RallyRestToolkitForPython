...
##
    ...
    def addCollectionItems(self, target, items):
        ...
        payload = {"CollectionItems":[{'_ref' : "%s/%s" % (str(item._type), str(item.oid))} 
                    for item in items]}
        #if target is feature change type to include "feature" subtype
        if target_type == 'PortfolioItem' and target.FormattedID.startswith('F'):
             collection_url = collection_url.replace('PortfolioItem','PortfolioItem/feature')
...
##
    ...
    def getAttachment(self, artifact, filename):
        ...
        att.Content = base64.b64decode(att_content.Content)  # maybe further txfm to Unicode ?
        return att
